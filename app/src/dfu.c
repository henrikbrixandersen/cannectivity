...
static unsigned int dfu_button_polls;
...
static void dfu_button_poll(struct k_work *work)
{
	...
	if (err > 0) {
		...
		if (++dfu_button_polls >= DFU_BUTTON_POLL_TOTAL) {
			LOG_INF("rebooting");
				 ...
				 gpio_port_pins_t pins)
{
	...
	dfu_button_polls = 0;
	k_work_reschedule(&dfu_button_work, K_MSEC(DFU_BUTTON_POLL_INTERVAL_MS));
}

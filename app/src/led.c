...
/* Activity LEDs by index */
enum {
	LED_ACTIVITY_RX_IDX = 0U,
	LED_ACTIVITY_TX_IDX,
	LED_ACTIVITY_COUNT,
};
...
#define LED_CTX_HAS_ACTIVITY_LED(_led_ctx)                                                         \
	(_led_ctx->activity_led[LED_ACTIVITY_RX_IDX].port != NULL)
#define LED_CTX_HAS_DUAL_ACTIVITY_LEDS(_led_ctx)                                                   \
	(_led_ctx->activity_led[LED_ACTIVITY_TX_IDX].port != NULL)
...
static void led_set_activity_rx(struct led_ctx *lctx, bool activity)
{
	int err;

	if (LED_CTX_HAS_ACTIVITY_LED(lctx)) {
		err = gpio_pin_set_dt(&lctx->activity_led[LED_ACTIVITY_RX_IDX], activity);
		if (err != 0) {
...
static void led_set_activity_tx(struct led_ctx *lctx, bool activity)
{
	struct gpio_dt_spec *led = NULL;
	int err;

	if (LED_CTX_HAS_DUAL_ACTIVITY_LEDS(lctx)) {
		led = &lctx->activity_led[LED_ACTIVITY_TX_IDX];
	} else if (LED_CTX_HAS_ACTIVITY_LED(lctx)) {
		led = &lctx->activity_led[LED_ACTIVITY_RX_IDX];
	}
...
static void led_state_normal_stopped_entry(void *obj)
{
	...
	led_set_activity_rx(lctx, false);
	led_set_activity_tx(lctx, false);
}
...
static void led_state_normal_started_run(void *obj)
{
	...
	switch (lctx->event) {
	case LED_EVENT_TICK:
		/* TODO: handle TX activity */
		if (lctx->ticks != 0U) {
			lctx->ticks--;
			if (lctx->ticks == (LED_TICKS_ACTIVITY / 2U)) {
				if (LED_CTX_HAS_ACTIVITY_LED(lctx)) {
					led_set_activity_rx(lctx, true);
				} else {
					led_set_state(lctx, false);
				}
...
static void led_state_identify_run(void *obj)
{
	...
	case LED_EVENT_TICK:
		if (--lctx->ticks == 0U) {
			...
			leds[1] = &lctx->activity_led[LED_ACTIVITY_RX_IDX];
			leds[2] = &lctx->activity_led[LED_ACTIVITY_TX_IDX];
